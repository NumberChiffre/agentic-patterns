...
- Next up (in order):
  1) Tier 0: Router/Feature/Reward/State/Cache interfaces + typed settings + retries/timeouts/budgets.
  2) Tier 1: Adaptive preview length; latency feature; fallback penalty; speculative top-2 full stage.
  3) Tier 2: Error/guardrail features; cold-start priors; failure classification; preview cache.
  4) Tier 3: Reward shaping; intent-aware features; learned fallback ordering; Thompson Sampling baseline.
...
- Observability: metrics interface (OpenTelemetry optional), structured logs; zero hard dependency on external services.
...
- Router telemetry: log context x, chosen arm, predicted mean/uncertainty, realized reward; export to wandb.
...
- Mitigation (Tier 1-2):
  - Add semantic embeddings feature to context. Use a small sentence encoder; reduce to 16–32 dims via projection to keep `d` small.
...
- Implementation notes:
  - New `src/features.py`: `compute_context_features(query, length_threshold)` returning `[1.0, length_norm, word_norm, emb_1..emb_k]`.
  - Update `LinUCBRouter(d=k+3)` and set `--bandit-length-threshold` appropriately.

### 2) Reward design is shaky (judge vs speed)
- Mitigation (Tier 2):
  - Use normalized judge overall as primary signal. Add optional latency and cost-per-token terms; clip to [0,1].
...
- Implementation notes:
  ...
  - Log per-term contributions for observability.
...
- `src/race.py`: plug new `feature_extractor`, updated reward blend, hybrid gate, logging.
